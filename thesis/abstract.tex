\begin{abstract}
%Problem
The increasing amount of data available to process in today's world led the emergence of engines for large-scale data processing such as Spark. In order to make querying more efficient, Rumble is an engine that automatically runs queries on top of Spark, using JSONiq as declarative querying language instead of Spark API. 
%Background
Rumble is still in its beta version and requires a testing mechanism that would verify behavior of its implementation. 
