...
While performing iterations, we had to ensure that any further implementation would not break the test-cases that were passing before. Thus, with every new iteration, we have performed a check that compared whether all the passed tests from the previous implementation were also contained in the new implementation or not. \todo{fix this text when you are inspired. Explain Comparing for new crashes and tests from OneNote}For each iteration we have maintained log files of all Success and Crashed test-cases. In every next iteration we have done two comparison between new and previous log files. By doing so we have created "List of test cases that were passing before but not anymore" and "Tests that were not crashing before, but are now and not in list above".
...
The main issue of Converter described in Section \ref{Phase2_Converter} was that it was performed via Java String.replace method. 
...
\section{JSONiq and Test Converter Data Format}
