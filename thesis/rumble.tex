...
\begin{table}[h!]
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|l|l|}
			\hline
			\textbf{Category}                    & \textbf{Expression/Clause}                                              \\ \hline
			\multirow{2}{*}{local-only}          & \multirow{2}{*}{Arithmetic, Logic, Comparison, Literal}                 \\
			&                                                                         \\ \hline
			sequence- transforming               & \$seq[...], \$a{[}\$i], \$a{[}{]}, \$a[[]], \$o.\$s, \$seq!..., annotate, treat \\ \hline
			\multirow{2}{*}{sequence- producing} & json-file, parquet-file, libsvm-file, text-file, csv-file               \\ \cline{2-2} 
			& avro-file, root- file, structured-json-file, parallelize                \\ \hline
			\multirow{3}{*}{sequence- combining} & seq1,\$seq2, if (\$c) then... else...,                                    \\ \cline{2-2} 
			& switch (\$x) case... default...,                                        \\ \cline{2-2} 
			& typeswitch (\$x) case... default...                                     \\ \hline
			FLWOR                                & FLWOR                                                                   \\ \hline
		\end{tabular}%
	}
...
\todo{Original thesis has FLWOR put in the group of Iterators that execute on top of Spark. However, ML on Rumble thesis puts FLWOR in the local runtime iterators. It is connected with previous question that was confusing me when I did the mapping. Second issue is both thesis do not match what is written in the code. In the code we have HybridRuntimeIterator}
